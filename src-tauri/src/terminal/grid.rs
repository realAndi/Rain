...
impl Row {
    ...
    pub fn to_styled_spans(&self) -> Vec<StyledSpan> {
        ...
        let mut cur_fg = self.cells[0].fg;
        let mut cur_bg = self.cells[0].bg;
        let mut cur_attrs = self.cells[0].attrs;
...
impl Grid {
    ...
    pub fn erase_cells(&mut self, row: u16, start_col: u16, end_col: u16) {
